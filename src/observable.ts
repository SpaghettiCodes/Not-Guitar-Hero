        ...
        (prev: State): State => {
            ...
            if (
                ...
                return {
                    ...
                    gameFrame: {
                        ...prev.gameFrame,
                        [key]: lineRemoveSpecific(lineAssociated, firstElement),
                    },
                    data: {
                        ...prev.data,
    ...
    const checkHitDetection =
        ...
        (prev: State): State => {
            ...
            const newScores = {
                multiplier: newMultiplier,
                score:
                    prev.data.score + ScoreConstant.BASE_SCORE * newMultiplier,
                combo: newCombo,
            };
            ...
            return {
                ...
                gameFrame: {
                    ...
                    [key]: isStream
                        ...
                        : lineRemoveSpecific(lineAssociated, firstElement),
                },
                data: {
                    ...prev.data,
...
const createNoteStream = (
    ...
    const firstNoteStart = Number(processedCSV.at(0)?.start);
    const delayBeginAmt = Math.max(
        TimeConstant.DELAY_SEC * 1000,
        maxTravelTime - firstNoteStart,
    );
    const maxDuration = Math.max(...processedCSV.map((x) => x.end));
    // appends a playable node to state
    // calls addPlayableNode for gameframe and
    // returns a new State with the new Gameframe
    const appendPlayableNode = (music: Music, state: State): State => {
        return {
            ...state,
...
const createTickStream = (): Observable<(state: State) => State> => {
    // gets the list of missed notes from a game frame
    ...
    const missedNotes = (prev: GameFrame) =>
        Array(
            ...
            (missed: ReadonlyArray<Note>, line: Line): ReadonlyArray<Note> => {
    ...
    const missedNotesCount = (prev: GameFrame): number =>
